...
container = d3.select("#histogram").style("margin", "10px").attr("id", "chartContainer");

d3.csv("./data/personer_indkomst_bydel_procenter_T.csv", function (data) {
    barchartDataset = data.map(function (d) {
        return [d["group"], +d["Amager Øst"], +d["Amager Vest"], +d["Bispebjerg"], +d["Brønshøj-Husum"], +d["Indre By"], +d["Nørrebro"], +d["Østerbro"], +d["Valby"], +d["Vanløse"], +d["Vesterbro"], +d["Avg in CPH"]];
    });
    ...
    //Create SVG element
    barchartSvg = d3.select("body").select("#chartContainer").append("svg").attr("width", barChartw + barChartPadding.left).attr("height", barCharth);

    barchartSvg.selectAll("rect").data(barchartDataset).enter().append("rect").attr("x", function (d, i) {
    ...
    // add category labels
    catLabels = barchartSvg.selectAll("gText").data(barchartDataset).enter().append("svg:g").attr("transform", function (d, i) {
        return "translate(" + (i * (barChartw / barchartDataset.length) + (barChartw / barchartDataset.length - barPadding) / 2 + barChartPadding.left) + "," + (barCharth - 60) + ")";
    }).append("text").text(function (d) {
        return d[0];
    }).attr("font-family", "sans-serif").attr("font-size", "11px").attr("fill", "black").attr("text-anchor", "end").attr("transform", function (d, i) {
        return "translate(10,0) rotate(-15,0,0)";
    });

    // add value labels
    valueLabels = barchartSvg.selectAll("text.values").data(barchartDataset).enter().append("text").text(function (d) {
    ...
    // add axis label
    barchartSvg.append("text").attr("transform", "rotate(-90)").attr("y", 0).attr("x", 0 - barCharth / 2).attr("dy", "1em").style("text-anchor", "middle").text("Pct. of people");
    title = barchartSvg.append("text").attr("y", 40).attr("x", barChartw / 2 + barChartPadding.left / 2).attr("dy", "1em").style("text-anchor", "middle").text(titleFinder());

    contextTitle = barchartSvg.append("text").attr("y", 20).attr("x", barChartw / 2 + barChartPadding.left / 2).attr("dy", "1em").style("text-anchor", "middle").text(context);
    contextTitle.text(context);
    title.text(titleFinder());
});

var titleFinder = function titleFinder() {
    return categories[dataSelector - 1];
};
var transition = function transition() {
    // redraw columns
    barchartSvg.selectAll("rect").data(barchartDataset).transition().duration(500).attr("x", function (d, i) {
        return i * (barChartw / barchartDataset.length) + barChartPadding.left;
    }).attr("y", function (d) {
        return barCharth - barchartYScale(d[dataSelector]) - barChartPadding.bottom;
    }).attr("width", barChartw / barchartDataset.length - barPadding).attr("height", function (d) {
    ...
    // redraw value labels
    valueLabels.data(barchartDataset).transition().duration(500).text(function (d) {
        return formatter(d[dataSelector]);
    }).attr("text-anchor", "middle").attr("x", function (d, i) {
        return i * (barChartw / barchartDataset.length) + (barChartw / barchartDataset.length - barPadding) / 2 + barChartPadding.left;
    }).attr("y", function (d) {
...
var LoadAndTransition = function LoadAndTransition(fileName) {
...
d3.csv("./data/indbyggere.csv", function (data) {
    dataset = data.map(function (d) {
        return [{ key: d["Bydel"], value: +d["Personer"] }];
    });
    ...
    function plotColumns(selection) {
        ...
        // Define scale for x
        var xScale = d3.scale.ordinal().domain(d3.range(dataset.length)).rangeRoundBands([padding, width], 0.05);
        ...
        svg.selectAll("text.labels").data(dataset).enter().append("text").text(function (d) {
            return d[0].key;
        }).attr("text-anchor", "middle").attr("x", function (d, i) {
            return xScale(i) + xScale.rangeBand() / 2;
        }).attr("y", function (d) {
        ...
        svg.append("text").attr("transform", "rotate(-90)").attr("y", 0).attr("x", padding / 2 - height / 2).attr("dy", "1em").style("text-anchor", "middle").text("Population");

        svg.append("text") // text label for the x axis
        .attr("x", width / 2).attr("y", height - padding + 70).style("text-anchor", "middle").text("District");
    }
...
var ctf_bolig = function ctf_bolig(Almennyttigt_boligbyggeri, Ejerbolig, Privat_udlejning, Privat_andelsboligforening, Stat_og_kommune, m2_Under_40, m2_40_59, m2_60_79, m2_80_99, m2_100_119, m2_120_og_derover) {
...
var ctf_person = function ctf_person(Kategori, Alder15_19, Alder20_29, Alder30_64, mere65) {
    if (Kategori <= 6.5) {
        if (Alder20_29 <= 0.5) {
            if (Alder30_64 <= 0.5) {
                if (Kategori <= 4.5) {
...
var ctf_person2 = function ctf_person2(Dansk, Indvandrere, alder16_17, alder18_19, alder20_24, alder_25_29, alder_30_39, alder40_59, alder_60_66, Grundskole, Erhvervsfaglig, Erhvervsgymnasial, Almen_gymnasial, Bachelor, Mellemlang_videregående, lang_videregående) {
    if (Dansk <= 0.5) {
        if (Indvandrere <= 0.5) {
            if (Grundskole <= 0.5) {
                if (alder40_59 <= 0.5) {
            ...
            if (Grundskole <= 0.5) {
                if (alder40_59 <= 0.5) {
                    if (alder_60_66 <= 0.5) {
                        if (alder_30_39 <= 0.5) {
                            if (Bachelor <= 0.5) {
                                return [[5530, 5670, 6105, 1806, 5400, 11790, 4228, 3325, 6156, 7953]];
                            } else {
...
var ctf_civil_born = function ctf_civil_born(ægtepar_regPartnere, par_samboende, enlig_kvinde, enlig_mand, barn0, barn1, barn2, barn3, barn3mere) {
...
var indexOfMax = function indexOfMax(arr) {
    if (arr.length === 0) {
        return -1;
    }
...
var getLabel = function getLabel(arr) {
...
var forestVote = function forestVote(Almennyttigt_boligbyggeri, Ejerbolig, Privat_udlejning, Privat_andelsboligforening, Stat_og_kommune, m2_Under_40, m2_40_59, m2_60_79, m2_80_99, m2_100_119, m2_120_og_derover, Kategori, Alder15_19, Alder20_29, Alder30_64, mere65, Dansk, Indvandrere, alder16_17, alder18_19, alder20_24, alder_25_29, alder_30_39, alder40_59, alder_60_66, Grundskole, Erhvervsfaglig, Erhvervsgymnasial, Almen_gymnasial, Bachelor, Mellemlang_videregående, lang_videregående, ægtepar_regPartnere, par_samboende, enlig_kvinde, enlig_mand, barn0, barn1, barn2, barn3, barn3mere) {
...
function plotMap2(selection) {
    ...
    queue().defer(d3.json, "data/cph_districts.json").defer(d3.csv, "data/indbyggere.csv").await(ready);
    function ready(error, geo, citizens) {
        if (error) throw error;
        var rates = {};
        citizens.map(function (d) {
            var antal = d.Personer;
            var bydel = d.Bydel;
            var objekt = { "bydel": bydel, "personer": antal };
            rates[bydel] = antal;
            return false;
        });
        var states = topojson.feature(geo, geo.objects.bydele),
            state = states.features.filter(function (d) {
            return d.properties.bydel_nr === 9;
        })[0];
        ...
        g.attr("class", "region").selectAll("path").data(states.features).enter().append("path").attr("d", path).style("fill", function (d) {
            return color(rates[d.properties.navn]);
        })
        //Adding mouseevents
        .on("mouseover", function (d) {
            // show border
            d3.select(this).transition().duration(300).style('stroke', 'black').style('stroke-width', '1');
            // show tooltip
            div.transition().duration(300).style("opacity", 1);
            div.text(d.properties.navn + " : " + rates[d.properties.navn]).style("left", d3.event.pageX + "px").style("top", d3.event.pageY - 30 + "px");
        }).on("mouseout", function () {
            // hide border
            d3.select(this).transition().duration(600).style('stroke', 'red').style('stroke-width', '0');
            div.transition().duration(300).style("opacity", 0);
        });

        //TODO: add labels to all districts
        g.attr("class", "region").selectAll("text").data(states.features).enter().append("text").text(function (d) {
...
function plotMap(selection) {
    ...
    var zoom = d3.behavior.zoom().scaleExtent([0.5, 15]).on("zoom", zoomed);
